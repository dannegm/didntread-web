...
export const encode64 = (raw: string): string =>
    CryptoJS.enc.Base64.stringify(CryptoJS.enc.Utf8.parse(raw));

export const decode64 = (encoded: string): string =>
    CryptoJS.enc.Base64.parse(encoded).toString(CryptoJS.enc.Utf8);

export const tokenEncode = <T>(payload: T): string => {
...
export const tokenDecode = <T>(encoded: string): T | null => {
